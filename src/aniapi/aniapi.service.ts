...
export class AniApiService {
  ...
  async getAniInfo(owner: string, petId: string): Promise<JSON> {
    const parser = new xml2js.Parser({ explicitArray: false });
    ...
    let queryParams =
      ...
      '=' +
      encodeURIComponent(
        'ulKK8tC5tkD4PAdqkFiF5APBX0NJeTQs3ZfjlO3Tbi8C96+ymyrvxD0QAO/RBTZny/SO/99tjrtRRo+x6XpSAw==',
      ); // 실제 서비스 키로 대체
  ...
  // ${JSON.stringify(animalInfo.response.body.item)}`);
      ...
      console.log('aniapi Test', animalInfo.response.body);

      // 변환된 데이터를 반환합니다.
      return animalInfo.response.body;
    } catch (error) {
